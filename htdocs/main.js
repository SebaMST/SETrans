function validateRegister() {
  var a = document.getElementById("fullname-r").value;
  if (!/^([a-zA-Z ]){2,}$/.test(a)) {
    alert("Wrong full name format. It may only contain at least two letters, and \" \"!");
    return false;
  }
  ...
  a = document.getElementById("email-r").value;
  ...
  a = document.getElementById("phone-r").value;
  if (!/^[0-9]{3,20}$/.test(a)) {
  ...
  a = document.getElementById("agree-terms-r").checked;
  if (!a) {
